...
export class AddCouponCodesComponent implements OnInit {
  ...
  getCategory() {
    this.db.get_rqst("", "app_karigar/getCategory").subscribe((r) => {
  ...
  getProduct(id) {
    console.log(id);
    this.db
      .post_rqst({ category_id: id }, "app_karigar/getProduct")
      .subscribe((r) => {
        console.log(r);
        this.product = r["productData"];
      });
  }
  ...
  getAvailableCoupanList(f) {
    ...
    this.db
      .post_rqst({ filter: this.filter }, "app_master/coupon_history")
      .subscribe((r) => {
        // console.log(r);
  ...
  saveCouponfrom(form: any) {
    ...
    if (this.coupon.total_coupon <= 2500) {
      ...
      console.log(this.coupon);
      this.db
        .post_rqst({ coupon: this.coupon }, "app_master/generate_coupon")
        .subscribe((d) => {
      ...
      this.dialog.error(
        "Can not Generate more than 2500 coupon codes at once!"
      );
    }
  }
  ...
  downloadCoupon(id) {
    this.db.post_rqst({ id: id }, "app_master/exportCoupon").subscribe((d) => {
      document.location.href =
        this.db.myurl + "/app/uploads/exports/coupons.csv";
  ...
  deleteCoupon(id) {
    this.dialog.delete("Coupon").then((result) => {
      if (result) {
        this.db
          .post_rqst({ id: id }, "app_master/delete_multi_coupon")
          .subscribe((d) => {
            console.log(d);
            this.getAvailableCoupanList("");
