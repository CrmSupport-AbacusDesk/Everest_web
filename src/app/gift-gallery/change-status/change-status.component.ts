...
export class ChangeStatusComponent implements OnInit {
    ...
    constructor(public db: DatabaseService, private route: ActivatedRoute, private router: Router,  public dialog: DialogComponent,
        ...
        {
            this.savingData = true;
            this.db.post_rqst( { 'desc' : this.support.desc ,'status' : this.support.status,'id': this.data.id }, 'app_karigar/change_support_status')
            .subscribe( d => {
                this.savingData = false;
                this.dialog.success( 'Status successfully Changed');
                this.dialogRef.close(true);
                console.log( d );
            });
